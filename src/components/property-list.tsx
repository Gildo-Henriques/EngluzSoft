...
export default function PropertyList({ properties, onDelete, onAddProperty }: PropertyListProps) {
  return (
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-2 flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">Preço: {property.price}</p>
                    <p className="text-gray-600">Quartos: {property.bedrooms}</p>
                    <p className="text-gray-600">Banheiros: {property.bathrooms}</p>
                    <p className="text-gray-600">Área: {property.size}</p>
                    <p className="text-gray-600">Localização: {property.location}</p>
                    {property.garage && (
                      <p className="text-gray-600">Garagem: {property.garage} vaga(s)</p>
                    )}
                    {property.yearBuilt && (
                      <p className="text-gray-600">Ano de Construção: {property.yearBuilt}</p>
                    )}
                  </div>
                  {property.images.length > 1 && (
                    <div className="grid grid-cols-3 gap-2">
                      {property.images.slice(1).map((photo, index) => (
                        <Image
                          key={index}
                          src={photo}
